...
import { IoIosArrowDown, IoIosArrowUp, IoIosCheckmarkCircle, IoIosCheckmarkCircleOutline, IoIosCloseCircle } from "react-icons/io";
import { MdCheckBox, MdOutlineCheckBoxOutlineBlank } from "react-icons/md";
import { displayDate } from "../../helpers/dateHelpers";
import { ManagedCourse_Course_by_pk_CourseEnrollments, ManagedCourse_Course_by_pk_CourseEnrollments_User } from "../../queries/__generated__/ManagedCourse";
...
export const ApplicationRow: FC<IProps> = ({
    ...
    return (<>
        {
            !enrollment && <div className="grid grid-cols-24 mb-1">
        ...
        {
            enrollment && <> <div className={`grid grid-cols-24 bg-edu-light-gray ${isRowOpen ? '' : 'mb-1'}`}>
                <div className="mr-3 ml-3 col-span-3">
                ...
                <div className="mr-3 ml-3 col-span-2 text-center">
                    {(!isExpired(enrollment) && enrollment.status === "APPLIED") && <GoPrimitiveDot className="inline" title="Beworben" color="grey" size="2.5em" />}
                    {(!isExpired(enrollment) && enrollment.status === "INVITED") && <IoIosCheckmarkCircle className="inline" title="Eingeladen" color="grey" size="1.5em" />}
                    {(enrollment.status === "CONFIRMED" || enrollment.status === "COMPLETED") && <IoIosCheckmarkCircle className="inline" title="Einladung bestätigt" color="lightgreen" size="1.5em" />}
                    {enrollment.status === "ABORTED" && <IoIosCheckmarkCircle title="Abgebrochen" color="red" size="1.5em" className="inline" />}
                    {enrollment.status === "REJECTED" && <IoIosCloseCircle title="Abgelehnt" color="red" size="1.5em" className="inline" />}
                    {(isExpired(enrollment) && (enrollment.status === "APPLIED" || enrollment.status === "INVITED")) && <IoIosCloseCircle className="inline" title="Einladung abgelaufen" color="grey" size="1.5em" />}
                </div>
                ...
                {isRowOpen && <div className="mb-1">
                    <div className="grid grid-cols-12 bg-edu-light-gray pt-5 pb-5">
                        ...
                        <div className="mr-3 ml-3 col-span-3">
                            <div>
                                <EhDot onClick={setUnrated} className="cursor-pointer" color="GREY" size={enrollment.motivationRating === "UNRATED" ? "LARGE" : "DEFAULT"} />
                                <EhDot onClick={setInvite} className="cursor-pointer" color="GREEN" size={enrollment.motivationRating === "INVITE" ? "LARGE" : "DEFAULT"} />
                                <EhDot onClick={setReview} className="cursor-pointer" color="ORANGE" size={enrollment.motivationRating === "REVIEW" ? "LARGE" : "DEFAULT"} />
                                <EhDot onClick={setDecline} className="cursor-pointer" color="RED" size={enrollment.motivationRating === "DECLINE" ? "LARGE" : "DEFAULT"} />
                            </div>

                            {enrollment.status === "INVITED" && <div className="mt-5">
