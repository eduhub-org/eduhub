...
import { ManagedCourse, ManagedCourseVariables, ManagedCourse_Course_by_pk } from "../../queries/__generated__/ManagedCourse";
...
function determineMaxAllowedTab(courseStatus: CourseStatus_enum) {
    let maxAllowedTab = 0;
    switch(courseStatus) {
...
function determineTabClasses(tabIndex: number, selectedTabIndex: number, courseStatus: CourseStatus_enum) {
...
export const AuthorizedManageCourse: FC<Props> = ({courseId}) => {
    ...
    const qResult = useAdminQuery<ManagedCourse, ManagedCourseVariables>(MANAGED_COURSE, {
        variables: {
            id: courseId
        }
    });
