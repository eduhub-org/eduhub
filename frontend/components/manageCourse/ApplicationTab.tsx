...
import { ManagedCourse_Course_by_pk, ManagedCourse_Course_by_pk_CourseEnrollments, ManagedCourse_Course_by_pk_CourseEnrollments_User } from "../../queries/__generated__/ManagedCourse";
import { ApplicationRow } from "./ApplicationRow";
import { greenDot, greyDot, orangeDot, redDot } from "../common/dots";
import { identityEventMapper, pickIdPkMapper, useAdminMutation, useRoleMutation, useUpdateCallback2 } from "../../hooks/authedMutation";
import { UpdateEnrollmentRating, UpdateEnrollmentRatingVariables } from "../../queries/__generated__/UpdateEnrollmentRating";
import { UPDATE_ENROLLMENT_FOR_INVITE, UPDATE_ENROLLMENT_RATING } from "../../queries/insertEnrollment";
...
import { UpdateEnrollmentForInvite, UpdateEnrollmentForInviteVariables } from "../../queries/__generated__/UpdateEnrollmentForInvite";
...
export const ApplicationTab: FC<IProps> = ({
    course,
    qResult
}) => {
    ...
    const handleSetInviteExpireDate = useCallback((d: Date | null) => {
    ...
    const [updateEnrollmentForInvite] = useRoleMutation<UpdateEnrollmentForInvite, UpdateEnrollmentForInviteVariables>(UPDATE_ENROLLMENT_FOR_INVITE, {
        context: {
            role: userRole
        }
    });
    const handleSendInvites = useCallback(async () => {
        ...
        if (mailTemplates != null) {
            ...
            if (inviteTemplate != null) {
                ...
                const relevantEnrollments = selectedEnrollments.map(eid => {
                    const ce = course.CourseEnrollments.find(e => e.id === eid);
                    return ce;
                }).filter(x => x != null && ["APPLIED", "INVITED", "REJECTED"].includes(x.status)) as ManagedCourse_Course_by_pk_CourseEnrollments[];
                ...
                try {
                    for (const enrollment of relevantEnrollments) {
                        ...
                        const doReplace = (source: string) => {
                            return source.replaceAll("[User:Firstname]", enrollment.User.firstName)
    ...
    }, [mailTemplates, selectedEnrollments, insertMailLogMutation, updateEnrollmentForInvite, qResult, setIsInviteDialogOpen]);

    const handleSelectRow = useCallback((enrollmentId: number, selected: boolean) => {
        if (selected) {
