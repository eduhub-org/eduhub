...
import { SelectComponentProperty, StaticComponentProperty } from '../../../types/UIComponents';
...
import { UnversityByComment, UnversityByComment_University } from '../../../queries/__generated__/UnversityByComment';
import { UNIVERSITY_LIST } from '../../../queries/university';
import Loading from '../../common/Loading';
import { EMPLOYMENT_LIST } from '../../../queries/employment';
import { EmplomentByValue, EmplomentByValue_Employment } from '../../../queries/__generated__/EmplomentByValue';
import { useAdminMutation } from '../../../hooks/authedMutation';
import { InsertSingleUser, InsertSingleUserVariables } from '../../../queries/__generated__/InsertSingleUser';
import { INSERT_A_USER } from '../../../queries/insertUser';
import { Employment_enum, University_enum } from '../../../__generated__/globalTypes';
...
const AddUser: FC<IProps> = ({ t, handleCancel }) => {
  return <UniversityAndEmploymentStatusLoader t={t} handleCancel={handleCancel} />;
};
export default AddUser;

const UniversityAndEmploymentStatusLoader: FC<IProps> = ({ t, handleCancel }) => {
  const unersityListRequest = useAdminQuery<UnversityByComment>(UNIVERSITY_LIST);
  ...
  return (
    <>
      {employmentList.length > 0 && universites.length > 0 && (
        <AddUserForm t={t} handleCancel={handleCancel} universities={universites} employmentList={employmentList} />
      )}
    </>
  );
};
...
const AddUserForm: FC<IPropsFrom> = ({ t, handleCancel, universities, employmentList }) => {
  ...
  return (
    <>
      <div className="flex flex-col space-y-5 p-10">
        {insertError ? <div className="pt-8 text-red-500">{insertError.graphQLErrors[0].message}</div> : null}
        <div className="grid grid-cols-12 gap-5">
          ...
          <div className="flex flex-col space-y-5 col-span-7">
            <input
              ...
              value={firstName}
              onChange={useCallback((e: ChangeEvent<HTMLInputElement>) => setFirstName(e.target.value), [setFirstName])}
            />
            <input
              className={inputBoxClass}
              placeholder={`${t('lastName')}*`}
              value={lastName}
              onChange={useCallback((e: ChangeEvent<HTMLInputElement>) => setLastName(e.target.value), [setLastName])}
            />
            <input
              className={inputBoxClass}
              value={email}
              onChange={useCallback((e: ChangeEvent<HTMLInputElement>) => setEmail(e.target.value), [setEmail])}
              placeholder={`${t('email')}*`}
            />
            <EhPassword
              property={{
            ...
            />
            <UserSelect componentProperty={employmentSelectProperty} selelectedValue={status} />
            <UserSelect componentProperty={propertyOfUniversityCheckBox} selelectedValue={school} />
            ...
            />
            <EhCheckBox2 onClickHandler={onAdministratorCheck} property={adminCheckBox} />
          </div>
        </div>
        <div className="flex justify-center py-5">
          {loading ? <Loading /> : <EhButton buttonText={t('save')} onClickCallback={handleSave} />}
        </div>
      </div>
    </>
  );
...
const UserSelect: FC<IProsSelect> = ({ componentProperty, selelectedValue }) => {
  const onSelectChanged = useCallback(
    (event: ChangeEvent<HTMLSelectElement>) => {
      componentProperty.onChangeHandler(componentProperty.componentID, event.target.value);
    },
    [componentProperty]
  );
  ...
  return (
    <select className={selectStyle} onChange={onSelectChanged} value={selelectedValue ?? ''}>
