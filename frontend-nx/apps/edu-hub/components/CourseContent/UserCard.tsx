...
const UserCard: FC<UserCardProps> = ({ user, role, className, size = 'large' }) => {
  ...
  const getProfileLink = (url: string) => {
    if (url.includes('linkedin.com')) {
        ...
        return <a href={url} target="_blank" rel="noopener noreferrer" className="underline">{t('external_profile')}</a>;
    }
  };
    ...
    return (
      <div className={className}>
        ...
        <div className={`flex flex-col ${fontSize}`}>
          ...
          {isLinkFormat(user.externalProfile) && <span className="text-gray-500">{getProfileLink(user.externalProfile)}</span>}
        </div>
      </div>
    );
  };
