...
const isPublished = (degreeCourse) =>
  degreeCourse?.Course?.published && degreeCourse?.Course?.Program?.published;
...
export const CompletedDegreeCourses: FC<{ degreeCourseId: number }> = ({
  degreeCourseId,
}) => {
  const { t } = useTranslation('course');
        ...
        <ul className="list-disc pb-12">
          {completedDegreeEnrollments.map((degreeEnrollment) => (
            <li key={degreeEnrollment?.Course?.id}>
              <NextLink
                href={`/course/${degreeEnrollment?.Course?.id}`}
                passHref
              >
                <MuiLink style={{ color: '#9CA3AF' }}>
                  {degreeEnrollment?.Course?.title} -{' '}
                  {degreeEnrollment?.Course?.Program?.shortTitle !== 'EVENTS'
                    ? ` ${t(degreeEnrollment?.Course?.Program?.title)} (${t(
                        degreeEnrollment?.Course?.ects
                      )} ECTS)`
