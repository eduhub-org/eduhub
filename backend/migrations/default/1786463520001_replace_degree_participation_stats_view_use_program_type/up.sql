...
-- COURSES / EVENTS / DEGREES. This view still used Program."shortTitle", which is
-- an editable free-text label: an organization that renames its programs silently
-- loses its degree statistics (no row at all for a renamed DEGREES program, zero
-- events for a renamed EVENTS program). Since the certificate requirement gate is
-- checked against the same rule these numbers express, the two must not be able
-- to disagree.
--
