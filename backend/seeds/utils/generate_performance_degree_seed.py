...
ATTENDANCE_COURSE_ID = 8200
ATTENDANCE_SESSION_ID_BASE = 82_000
ATTENDANCE_SESSION_COUNT = 24
ATTENDANCE_PARTICIPANT_LIMIT = 1000
ALL_LINKED_COURSE_IDS = COURSE_IDS + EVENT_COURSE_IDS + [ATTENDANCE_COURSE_ID]
...
ATTENDANCE_ENROLLMENT_ID_BASE = 240_000_000
ATTENDANCE_AUTOMATED_ID_BASE = 300_000_000
ATTENDANCE_INSTRUCTOR_ID_BASE = 400_000_000
ATTENDANCE_LATEST_INSTRUCTOR_ID_BASE = 500_000_000
...
def static_seed_sql(user_count: int) -> str:
    ...
    linked_course_ids_sql = ", ".join(str(course_id) for course_id in ALL_LINKED_COURSE_IDS)
    ...
    course_rows.append(
        (
            ...
            "Performance Attendance Course",
            "APPLICANTS_INVITED",
            "5",
            "Large attendance history for manual attendance performance testing.",
            "EN",
            "2026-12-31",
            "0",
            True,
            ...
            None,
            None,
            min(user_count, ATTENDANCE_PARTICIPANT_LIMIT),
            "18:00:00",
            "16:00:00",
            True,
            None,
...
WHERE id BETWEEN 81001 AND 81005
   OR id BETWEEN {ATTENDANCE_SESSION_ID_BASE + 1}
                     AND {ATTENDANCE_SESSION_ID_BASE + ATTENDANCE_SESSION_COUNT}
   OR "courseId" IN ({event_course_ids_sql}, {ATTENDANCE_COURSE_ID});
...
SELECT 8000 + row_number() OVER (),
       ...
       '2026-05-30 10:00:00+00'::timestamptz
FROM unnest(ARRAY[{linked_course_ids_sql}]) AS linked_course(course_id)
ON CONFLICT (id) DO NOTHING;
""".strip().format(linked_course_ids_sql=linked_course_ids_sql)
        + "\n"
    )
...
""".strip()
    ...
    sql.append(
        f"""
INSERT INTO public."Session" (id, title, description, "startDateTime", "endDateTime", "courseId", created_at, updated_at, "attendanceData", questionaire_sent)
SELECT {ATTENDANCE_SESSION_ID_BASE} + session_series.session_index,
       ...
       NULL,
       true
FROM generate_series(1, {ATTENDANCE_SESSION_COUNT}) AS session_series(session_index)
ON CONFLICT (id) DO NOTHING;
""".strip()
        + "\n"
    )
...
def attendance_performance_sql(user_count: int) -> str:
    participant_count = min(user_count, ATTENDANCE_PARTICIPANT_LIMIT)
    generated_user_id = (
        "('91000000-0000-0000-0000-' || "
        "lpad(participant_index::text, 12, '0'))::uuid"
    )
...
SELECT {ATTENDANCE_ENROLLMENT_ID_BASE} + participant_index,
       {ATTENDANCE_COURSE_ID},
       {generated_user_id},
       'CONFIRMED',
...
SELECT {ATTENDANCE_AUTOMATED_ID_BASE} + (participant_index * 100) + session_index,
       {ATTENDANCE_SESSION_ID_BASE} + session_index,
       {generated_user_id},
       ...
       participant_index % 3
FROM generate_series(1, {participant_count}) AS participant_series(participant_index)
CROSS JOIN generate_series(1, {ATTENDANCE_SESSION_COUNT}) AS session_series(session_index);

INSERT INTO public."Attendance" (id, "sessionId", "userId", status, created_at, updated_at, "recordedIdentifier", source, "startDateTime", "endDateTime", "totalAttendanceTime", "interruptionCount")
SELECT {ATTENDANCE_INSTRUCTOR_ID_BASE} + (participant_index * 100) + session_index,
       {ATTENDANCE_SESSION_ID_BASE} + session_index,
       {generated_user_id},
...
INSERT INTO public."Attendance" (id, "sessionId", "userId", status, created_at, updated_at, "recordedIdentifier", source, "startDateTime", "endDateTime", "totalAttendanceTime", "interruptionCount")
SELECT {ATTENDANCE_LATEST_INSTRUCTOR_ID_BASE} + (participant_index * 100) + session_index,
       {ATTENDANCE_SESSION_ID_BASE} + session_index,
       {generated_user_id},
...
def build_sql(user_count: int) -> str:
    ...
    sql += "\n"
    sql += attendance_performance_sql(user_count)
    sql += "\n"
    sql += insert_sql(
        "Attendance",
        [
