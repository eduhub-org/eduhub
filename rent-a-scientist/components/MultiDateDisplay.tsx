...
const rangeGroups = (indices: number[]) => {
    ...
    return indices.slice().sort().reduce((prev, cur) => {
        if (prev.length === 0) {
            return [[cur]];
        } else {
...
const rangeFormat = (range: number[], startDate: Date) => {
    if (range.length === 1) {
        ...
        return `${dayFormat(range[0], startDate)} bis ${dayFormat(range[1], startDate)}`;
    }
}
...
export const MultiDateSelector: FC<MultiDaySelectorProps> = ({
    ...
    const handleSelectDays = useCallback((event) => {
        if (onSelectDays) {
    ...
    const values = useMemo(() => {
        ...
        return mySelected.map(d => options.find(o => Number(o.value) === d)).filter(x => x != null);
