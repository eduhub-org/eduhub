...
export const TitledContent: FC<TitledContentProps> = ({ title, content }) => {
      ...
      <div className="lg:grid lg:grid-cols-2 border-t-4 border-rsa-background min-h-[400px]">
        <div>
          {offer.ScientistOfferRelations.map((rel, idx) => (
            <ScientistAvatar
              key={rel.Scientist.id}
              className="mt-4 pl-2 pr-2"
          ...
          <div className="mt-4 pl-5 lg:pl-0">
            ...
            <TitledContent
              title="Klassenstufen"
              content={
                offer.minimumGrade + ". bis " + offer.maximumGrade + ". Klasse"
              }
            />
            <MultiDateDisplay
            ...
            <TitledContent
              title="Zeitfenster"
              content={offer.timeWindow.join(", ")}
            />
            <TitledContent title="Dauer" content={offer.duration} />
            <TitledContent
              title="Einsatzort"
              content={offer.possibleLocations.join(", ")}
            />
            <TitledContent
              title="Equipment"
              content={offer.equipmentRequired}
            />
            <TitledContent
              title="Raumanforderungen"
              content={offer.roomRequirements}
            />
            <TitledContent
              title="Nötige Vorbereitung"
              content={offer.classPreparation}
            />
            <TitledContent
