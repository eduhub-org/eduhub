...
export const collectSchoolSubjects = (forOffers: AllScientistOffers_ScientistOffer[] | AllScientistOffers_ScientistOffer) => {
...
export const collectCategories = (forOffers: AllScientistOffers_ScientistOffer[] | AllScientistOffers_ScientistOffer) => {
    if (!Array.isArray(forOffers)) {
        forOffers = [forOffers];
    }
...
export const doRegionsMatch = (schoolPostalCode: string | undefined, region: string) => {
...
export const filterOffer = (offer: AllScientistOffers_ScientistOffer, filterCategories: string[], knownSchoolSubjects: string[], searchText: string, selectedGrade: number, schoolPostalCode: string | undefined) => {
    const o = offer;
    
    const allSchoolSubjects = offer.subjectComment == null || offer.subjectComment.length === 0;
    ...
    const oresult = (filterCategories.length === 0 || ((allSchoolSubjects && arraysIntersect(knownSchoolSubjects, filterCategories)) || arraysIntersect(offerCategories, filterCategories))) &&
    (textIntersect(o.description, searchText) || textIntersect(o.title, searchText)) &&
    o.minimumGrade <= selectedGrade &&
    o.maximumGrade >= selectedGrade &&
    o.possibleLocations.findIndex((l: string) => doRegionsMatch(schoolPostalCode, l)) !== -1
    return oresult;
}